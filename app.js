...
index 5d8b79b6fc3c5bc223ae497d0092de46c677e2be..2fdcf739ef4843b90729a7e90946872d97c595db 100644
--- a/app.js
+++ b/app.js
@@ -1,575 +1,507 @@
-const STORAGE_KEY = 'bulletin-app-items-v1';
+const STORAGE_KEY = 'bulletin-app-items-v2';
+const PREFS_KEY = 'bulletin-app-prefs-v1';
...
-    status: 'active',
-  },
+  filters: { query: '', priority: 'all', schedule: 'all', status: 'active' },
...
+    setTimeout(() => els.titleInput.focus(), 240);
   });
+
...
+    const item = getItemById(els.editId.value);
...
-    }
+    if (action === 'toggle-complete' && id) toggleComplete(id);
+    if (action === 'edit' && id) openEditModal(id);
+    if (action === 'delete' && id) deleteItem(id);
+    if (action === 'toggle-pin' && id) togglePin(id);
+    if (action === 'toggle-completed-section') state.completedCollapsed = !state.completedCollapsed;
...
+      setActiveTab('all');
+      renderApp();
+      const search = document.querySelector('[data-filter="query"]');
...
+    return { focusMode: !!parsed.focusMode };
+  } catch {
...
+function persistPrefs() { localStorage.setItem(PREFS_KEY, JSON.stringify(state.prefs)); }
+
...
-  });
+  [...els.priorityChips.querySelectorAll('.chip')].forEach(chip => chip.classList.toggle('active', chip.dataset.value === 'critical'));
...
-  persist();
-}
+function getItemById(id) { return state.items.find(item => item.id === id); }
+function deleteItem(id) { state.items = state.items.filter(item => item.id !== id); persist(); }
...
+  document.getElementById(`${view}View`).classList.add('active');
 ...
 function renderApp() {
   ...
   renderAllItems();
   renderSchedule();
+  els.focusModeBtn.textContent = state.prefs.focusMode ? 'Disable Focus Mode' : 'Enable Focus Mode';
   setActiveTab(state.currentView);
 }
...
+  const completedToday = state.items.filter(i => i.status === 'completed' && isTodayIso(i.completedAt));
   const critical = active.filter(i => i.priority === 'critical');
+  const overdue = active.filter(isOverdue);
+  const relief = Math.max(0, completedToday.length * 5);
...
+    { label: 'Mind Relief', value: `${relief}%` },
+  ].map(stat => `<div class="stat"><div class="label">${escapeHtml(stat.label)}</div><div class="value">${escapeHtml(String(stat.value))}</div></div>`).join('');
 }
...
+  const active = state.items.filter(i => i.status === 'active');
+  const focusFiltered = state.prefs.focusMode
+    ? active.filter(i => i.isPinned || ['critical', 'important'].includes(i.priority) || isOverdue(i) || isDueToday(i))
+    : active;
+
+  const pinned = sortItems(focusFiltered.filter(i => i.isPinned));
+  const topPriority = getTopPriority(focusFiltered.filter(i => !i.isPinned));
+  const dueSoon = sortItems(focusFiltered.filter(i => !i.isPinned && isDueToday(i)));
+  const unscheduledImportant = sortItems(focusFiltered.filter(i => !i.isPinned && !i.dueDate && ['critical', 'important'].includes(i.priority)));
+  const backlog = sortItems(focusFiltered.filter(i => !i.isPinned && !belongsToPrimarySections(i)));
+
+  const completed = sortItems(state.items.filter(i => i.status === 'completed')).reverse();
...
+    { title: 'Pinned Must-Do', items: pinned, empty: 'Pin tasks to keep them always visible.' },
+    { title: 'Top Priority', items: topPriority, empty: 'No immediate top-priority tasks.' },
+    { title: 'Due Today', items: dueSoon, empty: 'Nothing due today.' },
+    { title: 'Important Unscheduled', items: unscheduledImportant, empty: 'You are caught up on unscheduled important items.' },
+    { title: state.prefs.focusMode ? 'Focus Backlog' : 'Everything Else', items: backlog, empty: 'No backlog items at the moment.' },
   ];
...
+  els.bulletinView.innerHTML = sections.map(s => renderSection(s.title, s.items, s.empty)).join('') + renderCompletedSection(completed);
 }
...
+  return `<section class="section"><div class="section-header"><h3>${escapeHtml(title)}</h3><span class="section-count">${items.length}</span></div><div class="section-body">${items.length ? items.map(renderItemCard).join('') : `<div class="empty">${escapeHtml(emptyText)}</div>`}</div></section>`;
 }
...
-    </section>
-  `;
+  return `<section class="section ${state.completedCollapsed ? 'collapsed' : ''}"><div class="section-header"><h3>Completed</h3><div class="item-actions"><span class="section-count">${items.length}</span><button class="btn ghost" data-action="toggle-completed-section" type="button">${state.completedCollapsed ? 'Show' : 'Hide'}</button></div></div><div class="section-body">${items.length ? items.map(renderItemCard).join('') : '<div class="empty">Completed tasks will appear here.</div>'}</div></section>`;
 }
...
-    </div>
+    <div class="panel-title"><h3>All Items</h3><span class="tiny">Press / from anywhere to jump here instantly.</span></div>
     <div class="filters">
...
+      <select data-filter="priority">${renderOptions([['all','All priorities'],['critical','Critical'],['important','Important'],['normal','Normal'],['backBurner','Back Burner']], state.filters.priority)}</select>
+      <select data-filter="schedule">${renderOptions([['all','All scheduling'],['scheduled','Scheduled'],['unscheduled','Unscheduled']], state.filters.schedule)}</select>
+      <select data-filter="status">${renderOptions([['active','Active only'],['completed','Completed only'],['all','All statuses']], state.filters.status)}</select>
     </div>
+    <div class="section-body">${items.length ? items.map(renderItemCard).join('') : '<div class="empty">No tasks match these filters.</div>'}</div>
   `;
 }
 
 function renderSchedule() {
-  const activeScheduled = state.items.filter(item => item.status === 'active' && item.dueDate).sort(sortItemsComparator);
+  const activeScheduled = state.items.filter(i => i.status === 'active' && i.dueDate).sort(sortItemsComparator);
...
+  const later = activeScheduled.filter(i => !isDueToday(i) && !isThisWeekButNotToday(i));
...
-    </div>
+    <div class="panel-title"><h3>Schedule</h3><span class="tiny">Timeline view for calm planning.</span></div>
...
-    </div>
-  `;
+  return `<div class="agenda-group"><div class="agenda-heading">${escapeHtml(title)}</div><div class="section-body">${items.length ? items.map(renderItemCard).join('') : '<div class="empty">No tasks scheduled here.</div>'}</div></div>`;
 }
...
+
+  return `<article class="item-card ${completed ? 'completed' : ''} ${overdue ? 'overdue' : ''}">
+    <div class="item-top">
+      <button class="check ${completed ? 'done' : ''}" data-action="toggle-complete" data-id="${item.id}" type="button" aria-label="${completed ? 'Mark active' : 'Mark complete'}">${completed ? '✓' : ''}</button>
+      <div class="item-main">
+        <h4>${escapeHtml(item.title)}</h4>
...
+    </div>
+    <div class="item-actions">
+      <button class="btn ghost" data-action="toggle-pin" data-id="${item.id}" type="button">${item.isPinned ? 'Unpin' : 'Pin'}</button>
+      <button class="btn secondary" data-action="edit" data-id="${item.id}" type="button">Edit</button>
+      <button class="btn danger" data-action="delete" data-id="${item.id}" type="button">Delete</button>
+    </div>
+  </article>`;
...
-    );
-  }
+  if (q) items = items.filter(item => item.title.toLowerCase().includes(q) || (item.notes || '').toLowerCase().includes(q));
...
+  if (state.prefs.focusMode) items = items.filter(item => item.status === 'completed' || item.isPinned || ['critical', 'important'].includes(item.priority));
...
+  const overdueCritical = activeItems.filter(i => isOverdue(i) && i.priority === 'critical');
...
+  const todayImportant = activeItems.filter(i => isDueToday(i) && ['critical', 'important'].includes(i.priority));
+  if (todayImportant.length) return sortItems(todayImportant);
+  const pinned = activeItems.filter(i => i.isPinned);
+  if (pinned.length) return sortItems(pinned);
+  return sortItems(activeItems.filter(i => ['critical', 'important'].includes(i.priority))).slice(0, 5);
 }
...
-  );
+  return isOverdue(item) || isDueToday(item) || (!item.dueDate && ['critical', 'important'].includes(item.priority));
 }
...
+  const timeDiff = getDueTimestamp(a) - getDueTimestamp(b);
...
+  const date = parseLocalDate(item.dueDate).toLocaleDateString([], { month: 'short', day: 'numeric' });
+  const time = item.dueTime ? ` · ${formatTime(item.dueTime)}` : '';
+  if (isOverdue(item)) return `Overdue · ${date}${time}`;
+  if (isDueToday(item)) return `Today${time}`;
+  return `${date}${time}`;
 }
